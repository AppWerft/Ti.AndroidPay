...
#define TAG "ti.androidpay"

using namespace v8;

static Persistent<Object> bindingCache;

static void Androidpayment_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Androidpayment.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::AndroidpaymentBindings::lookupGeneratedInit(
...
static void Androidpayment_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Androidpayment_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Androidpayment_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::AndroidpaymentBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry AndroidpaymentBinding = {
	"ti.androidpay",
	Androidpayment_init,
	Androidpayment_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ti_androidpay_AndroidpaymentBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.androidpay", &AndroidpaymentBinding);
	titanium::KrollBindings::addExternalLookup(&(::AndroidpaymentBindings::lookupGeneratedInit));
}
